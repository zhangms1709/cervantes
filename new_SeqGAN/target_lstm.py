...
class TARGET_LSTM(object):
    def __init__(self, num_emb, batch_size, emb_dim, hidden_dim, sequence_length, start_token, params):
        ...
        # pretraining loss
        self.pretrain_loss = -tf.reduce_sum(tf.compat.v1.one_hot(tf.cast(tf.reshape(self.x, [-1]), tf.int32), tf.cast(self.num_emb, tf.int32)) * tf.math.log(
    tf.reshape(self.g_predictions, [-1, self.num_emb])) / (self.sequence_length * self.batch_size))

        tf.compat.v1.one_hot(tf.cast(tf.reshape(self.x, [-1]), tf.int32), tf.cast(self.num_emb, tf.int32), 1.0) * tf.math.log(
    tf.reshape(self.g_predictions, [-1, self.num_emb]))
        self.out_loss = tf.reduce_sum(tf.reduce_sum(tf.math.log(tf.compat.v1.one_hot(tf.cast(tf.reshape(self.x, [-1]), tf.int32), depth=self.num_emb, on_value=1.0, off_value=0.0)), axis=1), axis=0)
